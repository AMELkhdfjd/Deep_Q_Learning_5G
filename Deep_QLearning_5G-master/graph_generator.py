...
### changements: change the randomness to assign a type to the node, we have only central nodes and also the revenu to eliminate (we need to take into consideration the urllc)
import json
...
cpu_central = (300,300) #cpu range for centralized nodes, a tuple is used which is an immutable obj ATT random generates only 300, need to vary maybe ?
#str_central = (200,200)
#cpu_local = (150,150)
...
def ba_graph(name,n):
    ...
    for n in G.nodes():
        ...
        G.nodes[n]["type"] = node_type ## determines the type of the node if central or edge using the previous lists
        
        if node_type == 1: ## which means edge
            G.nodes[n]["cpu"] = random.randint(cpu_edge[0],cpu_edge[1]) ## generate the number 100 ATTTTTT
    ...
    G.graph["max_bw_profit"] = G.graph["max_bw_revenue"]-G.graph["min_bw_cost"] ### to eliminate in our solution for the reward
